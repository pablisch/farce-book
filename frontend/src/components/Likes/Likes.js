...
const Likes = ({ likes, parent }) => {
  ...
  const handleButtonClick = async (event) => {
    ...
    const response = await fetch(`${baseUrl}/posts/likes`, {
      method: 'PATCH',
      headers: {
  ...
  const userHasLiked = (likers) => {
    if (!likers) {
      return false;
    } // error handling for empty likes array
    let result = false;
  ...
  if (userHasLiked(likers)) {
    likeEmoji = (
      <img src={likedThumbsUp} alt='thumbs up liked emoji' className='thumb' />
    );
  } else {
  ...
  return (
    <div className='likes'>
      ...
      <div id='like-count' data-cy='like-count'>
        {' '}
        {likers && likers.length} {likesLabel}{' '}
      </div>
    </div>
  );
};
